...
Public Class frmEditPayments
    ...
    Private Sub frmEditPayments_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            str = "select * from paymentsaccounts where CollectionSource='Student\'s Account' and Sem='" & frmAccounting.stSemester.Text & "' and SY='" & frmAccounting.stSY.Text & "'"
            conn.Open()
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        Try
            str = "select * from paymentsaccounts where LastName like '%" & txtSearch.Text & "%' or ORNumber like '%" & txtSearch.Text & "%' and CollectionSource='Student\'s Account' and Sem='" & frmAccounting.stSemester.Text & "' and SY='" & frmAccounting.stSY.Text & "'"
            conn.Open()
            Dim mysda As New MySqlDataAdapter(str, conn)
            Dim ds As New DataSet
            mysda.Fill(ds, "paymentsaccounts")
    ...
    Private Sub dgStudeAccounts_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgStudeAccounts.DoubleClick
        ...
        frmPayment.txtDescription.Text = dgStudeAccounts.Item(8, i).Value


        Dim totalpayment, totalAccount As Double
        'Get Total Payments
        str = "select * from paymentsaccounts where StudentNumber='" & frmPayment.mskStudentNumber.Text & "' and Sem='" & frmAccounting.stSemester.Text & "' and SY='" & frmAccounting.stSY.Text & "' and ORStatus='Valid'"
        conn.Open()
