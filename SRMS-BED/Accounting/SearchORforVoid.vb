...
Public Class frmSearchORforVoid
    Dim str As String
    Private Sub frmSearchORforVoid_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            str = "select ID, ORNUmber, Amount, PDate, concat(s.FirstName,' ',s.MiddleName,' ',s.LastName) as Payor, Description, Cashier, s.StudentNumber from paymentsaccounts p join studeprofile s on p.StudentNumber=s.StudentNumber where ORStatus='Valid'"
            conn.Open()
    ...
    Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        Try
            str = "select ID, ORNUmber, Amount, PDate, concat(s.FirstName,' ',s.MiddleName,' ',s.LastName) as Payor, Description, Cashier, s.StudentNumber from paymentsaccounts p join studeprofile s on p.StudentNumber=s.StudentNumber where ORNumber like '%" & txtSearch.Text & "%' and ORStatus='Valid'"
            conn.Open()
            Dim mysda As New MySqlDataAdapter(str, conn)
            Dim ds As New DataSet
            mysda.Fill(ds, "paymentaccounts")
    ...
    Private Sub dg1_DoubleClick(sender As Object, e As EventArgs) Handles dg1.DoubleClick
        ...
        'Get Total Payments
        str = "select * from paymentsaccounts where StudentNumber='" & frmVoidOR.lblStudentNumber.Text & "' and Sem='" & frmAccounting.stSemester.Text & "' and SY='" & frmAccounting.stSY.Text & "' and ORStatus='Valid'"
        conn.Open()
        ...
        str = "select * from studeaccount where StudentNumber='" & frmVoidOR.lblStudentNumber.Text & "' and Sem='" & frmAccounting.stSemester.Text & "' and SY='" & frmAccounting.stSY.Text & "' group by AcctTotal"
        conn.Open()
